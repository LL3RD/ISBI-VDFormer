...
class CocoDataset_3slices(CocoDataset):
    CLASSES = ('tumour',)
    ...
    def prepare_train_img(self, idx):
        ...
        results['img']._data = torch.stack(
            (result_data[0], results['img'].data, result_data[1]), 0)
        return results
    ...
    def prepare_test_img(self, idx):
        ...
        for i in [-1, 1]:
            img_idx = img_id + i
            if img_idx < 0 or img_idx >= len(self.data_infos_all):
                data = torch.zeros(results['img'][0].shape)
                result_data.append(data)
                continue
            img_info = self.data_infos_all[img_idx]
        ...
        results['img'][0] = torch.stack(
            (result_data[0], results['img'][0], result_data[1]), 0)
        return results
