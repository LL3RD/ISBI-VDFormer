...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        assert L == S * H * W, print(L, " ", S, " ", H, " ", W)# "input feature has wrong size"
...
class SwinTransformer3dLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # print(pretrain_img_size[0] // patch_size[0] // 2 ** 0)
    ...
    def forward(self, x):
        """Forward function."""
        down = []
        x = x.view(-1, 256, 5, x.size(2), x.size(3)).contiguous()
        # print(x.shape)
        # print(x.shape)
        Ws, Wh, Ww = x.size(2), x.size(3), x.size(4)
        if self.ape:
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                down.append(out)
        return down[0][:, :, 2, :, :].contiguous()

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinTransformer3dLayer, self).train(mode)
        self._freeze_stages()